...
def model_fn(features, labels, mode, params):
  with tf.variable_scope('x_prep'):
    x = tf.reshape(features, [-1, 125, 161, 2], name='input_incep106_lg')
  ...
  branch_stack = branch_incep(x_norm, 'branch_stack', mode, params)
  ...
  with tf.variable_scope('branch_concat'):
    flat_stack = flatten(branch_stack, 'flat_stack')
    flat_spec = flatten(branch_spec, 'flat_spec')
    branch_merge = tf.concat([flat_stack, flat_spec, branch_freq], axis=1, name='branch_merge')
