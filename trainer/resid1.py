...
def model_fn(features, labels, mode, params):
    x = tf.reshape(features, [-1, 99, 161, 1], name='input_resid1')
    x_norm = tf.layers.batch_normalization(x, training=mode == tf.estimator.ModeKeys.TRAIN, name='x_norm')
    if params['verbose_summary']:
        tf.summary.image('input', x)

    conv = x_norm
    conv = resid_block(conv, 16, mode, 'conv_1')
    conv = resid_block(conv, 32, mode, 'conv_2')
    conv = resid_block(conv, 64, mode, 'conv_3')
