...
def model_fn(features, labels, mode, params):
    x = tf.reshape(features, [-1, 125, 161, 2], name='input_incep101_lg')
    ...
    norm = tf.layers.batch_normalization(pool1, training=mode == tf.estimator.ModeKeys.TRAIN, name='norm_incep2')
    incep2 = inception_block(norm, t1x1=8, t3x3=8, t5x5=8, tmp=8, name='incep2')
    ...
    norm = tf.layers.batch_normalization(pool3, training=mode == tf.estimator.ModeKeys.TRAIN, name='norm_conv5')
    conv5 = tf.layers.conv2d(norm, filters=64, kernel_size=3, padding='same', activation=tf.nn.relu, name='conv5')
    ...
    norm = tf.layers.batch_normalization(pool5, training=mode == tf.estimator.ModeKeys.TRAIN, name='norm_incep6')
    incep6 = inception_block(norm, t1x1=32, t3x3=32, t5x5=32, tmp=32, name='incep6')
    ...
    norm = tf.layers.batch_normalization(pool7, training=mode == tf.estimator.ModeKeys.TRAIN, name='norm_conv8')
    conv8 = tf.layers.conv2d(norm, filters=256, kernel_size=3, padding='same', activation=tf.nn.relu, name='conv8')
