...
def model_fn(features, labels, mode, params):
    ...
    dim = incep5.get_shape()[1:]
    dim = int(dim[0] * dim[1] * dim[2])
    flat = tf.reshape(incep5, [-1, dim], name='flat')
    dropout6 = tf.layers.dropout(flat, rate=params['dropout_rate'], training=mode == tf.estimator.ModeKeys.TRAIN, name='dropout6')
    dense6 = tf.layers.dense(dropout6, units=2048, activation=tf.nn.relu, name='dense6')

    logits = tf.layers.dense(dense6, units=params['num_classes'], name='logits')
