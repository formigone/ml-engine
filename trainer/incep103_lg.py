...
def branch(x, name, mode, params):
  with tf.variable_scope(name):
    ...
    flat = flatten(pool8)
    dense = tf.layers.dense(flat, units=2048, activation=tf.nn.relu, name='dense')
...
def model_fn(features, labels, mode, params):
  x = tf.reshape(features, [-1, 125, 161, 2], name='input_incep103_lg')
  ...
  flat_a = branch(x_a, 'branch_a', mode, params)
  flat_b = branch(x_b, 'branch_b', mode, params)
  branch_merge = tf.concat([flat_a, flat_b], 1, name='branch_merge')
