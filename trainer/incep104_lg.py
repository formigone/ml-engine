...
def model_fn(features, labels, mode, params):
  x = tf.reshape(features, [-1, 125, 161, 2], name='input_incep104_lg')
  ...
  branch_a = branch(x_a, 'branch_a', mode, params)
  branch_b = branch(x_b, 'branch_b', mode, params)
  branch_merge = tf.concat([branch_a, branch_b], axis=3, name='branch_merge')
  flat = flatten(branch_merge, 'branch_flatten')
  norm = tf.layers.batch_normalization(flat, training=mode == tf.estimator.ModeKeys.TRAIN, name='norm_merge')
