...
def model_fn(features, labels, mode, params):
    x = tf.reshape(features, [-1, 99, 161, 1], name='input_stric_conv1')
    x_norm = tf.layers.batch_normalization(x, training=mode == tf.estimator.ModeKeys.TRAIN, name='x_norm')
    if params['verbose_summary']:
        tf.summary.image('input', x)

    conv = x_norm
    conv = stric_block(conv, 16, mode, 'conv_1')
    conv = stric_block(conv, 32, mode, 'conv_2')
    conv = stric_block(conv, 64, mode, 'conv_3')
    conv = stric_block(conv, 128, mode, 'conv_4')
    conv = stric_block(conv, 256, mode, 'conv_5', only_same=True)
